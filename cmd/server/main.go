...
func main() {
	...
	clientManager := clients.ClientManager{make(map[string]*clients.Client), make(chan messages.ClientMessage)}
	...
	for {
		...
		client := clients.Client{guuid.New().String(), conn, make(chan []byte), make(chan []byte)}
